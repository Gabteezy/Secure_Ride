...
class ListOfCommuterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Set up RecyclerView
        binding.recyclerViewCommuters.apply {
    ...
    private fun fetchCommuters() {
        val commutersRef = FirebaseDatabase.getInstance().getReference("user").orderByChild("userType")
            .equalTo("Commuter")
        ...
        commutersRef.addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(
                    this@ListOfCommuterActivity,
                    "Failed to load commuters: ${error.message}",
                    Toast.LENGTH_SHORT
                ).show()
            }
        })
